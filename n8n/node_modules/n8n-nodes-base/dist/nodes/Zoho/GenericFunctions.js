...
async function zohoApiRequest(method, endpoint, body = {}, qs = {}, uri) {
    var _a;
    const { oauthTokenData } = this.getCredentials('zohoOAuth2Api');
    const options = {
        body: {
            data: [
                body,
...
exports.adjustAccountPayload = lodash_1.flow(adjustBillingAddressFields, adjustShippingAddressFields, adjustCustomFields);
exports.adjustContactPayload = lodash_1.flow(adjustMailingAddressFields, adjustOtherAddressFields, adjustDateOfBirthField, adjustCustomFields);
exports.adjustDealPayload = lodash_1.flow(adjustClosingDateField, adjustCustomFields);
exports.adjustInvoicePayload = lodash_1.flow(adjustBillingAddressFields, adjustShippingAddressFields, adjustInvoiceDateField, adjustDueDateField, adjustAccountIdField, adjustCustomFields);
exports.adjustInvoicePayloadOnUpdate = lodash_1.flow(exports.adjustInvoicePayload, exports.adjustProductDetailsOnUpdate);
exports.adjustLeadPayload = lodash_1.flow(adjustAddressFields, adjustCustomFields);
exports.adjustPurchaseOrderPayload = lodash_1.flow(adjustBillingAddressFields, adjustShippingAddressFields, adjustDueDateField, adjustPurchaseOrderDateField, adjustCustomFields);
exports.adjustQuotePayload = lodash_1.flow(adjustBillingAddressFields, adjustShippingAddressFields, adjustValidTillField, adjustCustomFields);
exports.adjustSalesOrderPayload = lodash_1.flow(adjustBillingAddressFields, adjustShippingAddressFields, adjustDueDateField, adjustAccountIdField, adjustContactIdField, adjustDealIdField, adjustCustomFields);
exports.adjustVendorPayload = lodash_1.flow(adjustAddressFields, adjustCustomFields);
...
exports.toLoadOptions = (items, nameProperty) => items.map((item) => ({ name: item[nameProperty], value: item.id }));
async function getFields(resource, { onlyCustom } = { onlyCustom: false }) {
    ...
    return lodash_1.sortBy(options, o => o.name);
}
...
function getSectionApiName(resource) {
    ...
    return `${exports.capitalizeInitial(resource)} Information`;
}
exports.addGetAllFilterOptions = (qs, options) => {
    if (Object.keys(options).length) {
        const { fields } = options, rest = __rest(options, ["fields"]);
        Object.assign(qs, fields && { fields: fields.join(',') }, rest);
    }
};
exports.capitalizeInitial = (str) => str[0].toUpperCase() + str.slice(1);
