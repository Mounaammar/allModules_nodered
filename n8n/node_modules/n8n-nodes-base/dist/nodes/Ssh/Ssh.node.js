    ...
    async execute() {
        ...
        try {
            if (authentication === 'password') {
                const credentials = this.getCredentials('sshPassword');
                await ssh.connect({
                ...
                const credentials = this.getCredentials('sshPrivateKey');
                const { path, } = await tmp_promise_1.file();
                ...
                try {
                    ...
                    if (resource === 'file') {
                        if (operation === 'download') {
                            ...
                            const { path } = await tmp_promise_1.file({ mode: 0x0777, prefix: 'prefix-' });
                            temporaryFiles.push(path);
                        ...
                        if (operation === 'upload') {
                            ...
                            const { path } = await tmp_promise_1.file();
                            temporaryFiles.push(path);
                            await promises_1.writeFile(path, Buffer.from(binaryData.data, n8n_core_1.BINARY_ENCODING));
