...
async function uploadAttachments(binaryProperties, items, i) {
    ...
    for (const binaryPropertyName of binaryProperties) {
        ...
        if (isImage) {
            ...
            attachmentBody = {
                command: 'INIT',
                total_bytes: Buffer.from(binaryData[binaryPropertyName].data, n8n_core_1.BINARY_ENCODING).byteLength,
                media_type: binaryData[binaryPropertyName].mimeType,
            };
            response = await twitterApiRequest.call(this, 'POST', '', {}, {}, uploadUri, { form: attachmentBody });
            const mediaId = response.media_id_string;
            const binaryParts = chunks(Buffer.from(binaryData[binaryPropertyName].data, n8n_core_1.BINARY_ENCODING), 5242880);
            let index = 0;
