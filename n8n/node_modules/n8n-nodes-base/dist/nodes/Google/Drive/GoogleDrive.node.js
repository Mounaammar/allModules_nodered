        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (resource === 'drive') {
                if (operation === 'create') {
                    ...
                    const response = await GenericFunctions_1.googleApiRequest.call(this, 'POST', `/drive/v3/drives`, body, { requestId: uuid_1.v4() });
            ...
            if (resource === 'file') {
                if (operation === 'copy') {
                    ...
                    const response = await GenericFunctions_1.googleApiRequest.call(this, 'GET', `/drive/v3/files/${fileId}`, {}, { alt: 'media' }, undefined, requestOptions);
                    let mimeType;
                    ...
                    const responseData = await GenericFunctions_1.googleApiRequest.call(this, 'PATCH', `/drive/v3/files/${id}`, {}, qs);
            ...
            if (['file', 'folder'].includes(resource)) {
                if (operation === 'delete') {
                    const fileId = this.getNodeParameter('fileId', i);
                    const response = await GenericFunctions_1.googleApiRequest.call(this, 'DELETE', `/drive/v3/files/${fileId}`);
                    returnData.push({
                        fileId,
                        success: true,
                    });
