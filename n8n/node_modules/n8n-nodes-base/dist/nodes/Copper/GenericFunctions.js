...
function adjustPhoneNumbers(fixedCollection) {
    if (!fixedCollection.phone_numbers)
        return fixedCollection;
    const adjusted = lodash_1.omit(fixedCollection, ['phone_numbers']);
    if (fixedCollection.phone_numbers) {
...
function adjustEmails(fixedCollection) {
    if (!fixedCollection.emails)
        return fixedCollection;
    const adjusted = lodash_1.omit(fixedCollection, ['emails']);
    if (fixedCollection.emails) {
...
function adjustProjectIds(fields) {
    if (!fields.project_ids)
        return fields;
    const adjusted = lodash_1.omit(fields, ['project_ids']);
...
function adjustEmail(fixedCollection) {
    if (!fixedCollection.email)
        return fixedCollection;
    const adjusted = lodash_1.omit(fixedCollection, ['email']);
    if (fixedCollection.email) {
...
exports.adjustCompanyFields = lodash_1.flow(adjustAddress, adjustPhoneNumbers);
exports.adjustLeadFields = lodash_1.flow(exports.adjustCompanyFields, adjustEmail);
exports.adjustPersonFields = lodash_1.flow(exports.adjustCompanyFields, adjustEmails);
exports.adjustTaskFields = lodash_1.flow(exports.adjustLeadFields, adjustProjectIds);
